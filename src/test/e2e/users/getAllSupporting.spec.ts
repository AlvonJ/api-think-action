...
describe('get all supportings example', () => {
  let app;
  ...
  it('should be able get all supporting', async () => {
    ...
    const response = await request(app)
      .get(`/v1/users/${data[0]._id.toString()}/supporting`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.statusCode).toEqual(200);
    expect(response.body.status).toEqual('success');
    expect(response.body.supportingCount).toBeDefined();
  ...
  it('should thrown Authentication Error if user is not logged in', async () => {
    const response = await request(app).get('/v1/users/supporting');
